...
def calculate_reliability(data):
	...
	if success_ratio == 1: # Always meets deadline
		...
		if success_ratio >= 0.90:
			return ("A: {}%".format(success_ratio * 100), "green")
		elif success_ratio >= 0.80:
			return ("B: {}%".format(success_ratio * 100), "green")
		elif success_ratio >= 0.70:
			return ("C: {}%".format(success_ratio * 100), "#EEC700")
		elif success_ratio >= 0.60:
			return ("D: {}%".format(success_ratio * 100), "#EEC700")
		else:
			return ("F: {}%".format(success_ratio * 100), "red")
...
def plot_traces(csv):
	...
	for index, row in testing.iterrows():
		trace = row['trace'][1:-1].split('), ')
		array = np.zeros((9, 9))
		for pos in trace:
			point_x, point_y = int(pos[1]), int(pos[4])
			array[point_x][point_y] = 1
		array[int(trace[0][1])][int(trace[0][4])] = 2 # start
		array[int(trace[-1][1])][int(trace[-1][4])] = 3 # end
		...
		ax[r][c].imshow(array)
		ax[r][c].invert_yaxis()
		autoAxis = ax[r][c].axis()
		color = 'red' if row['success'] == 0 else 'green'
		rec = patches.Rectangle((autoAxis[0]-0.7,autoAxis[2]-0.2),(autoAxis[1]-autoAxis[0])+1,(autoAxis[3]-autoAxis[2])+0.4,fill=False,edgecolor=color,lw=2)
		rec = ax[r][c].add_patch(rec)
